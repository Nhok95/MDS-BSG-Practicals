...
```{r question_2}
#dist.matrix <- as.matrix(dist(genotypeData, method = "manhattan"))
#dist.matrix[1:5,1:5]
```

```{r question_2_store, include=FALSE}
#saveRDS(dist.matrix, file = "dist_matrix.rds")
dist.matrix <- readRDS(file = "dist_matrix.rds")
```
...
```{r question_3}
as.matrix(dist(genotypeData[1:5,], method = "minkowski", p=1))
```
...
```{r question_4}
mds.out <- cmdscale(dist.matrix,eig=TRUE,k=1)
...
With k=2 we have a godness of fit of 0.17 that represents less than 20% of the population with 
