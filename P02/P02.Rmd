...
```{r options} 
options("scipen"=100, "digits"=3) #Default values
```
...
x <- MakeCounts(geneticData.poly$rs587756191_T, coding = c(0,1,2))[1,1:3]
...
```{r options} 
options("scipen"=0, "digits"=7) #Default values
```

## 4. Determine the genotype counts for all these variants, and store them in a p × 3 matrix.
```{r}
genotypeCounts <- MakeCounts(geneticData.poly, coding=c(0,1,2))[,1:3]
```
...
```{r}
Chisq.pvals <- HWChisqStats(genotypeCounts,pvalues=TRUE)
...
geneticData.poly[, which.max(Chisq.pvals), with=FALSE]
